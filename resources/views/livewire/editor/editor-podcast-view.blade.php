  ...
  <div class="py-10">
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:max-w-7xl lg:px-8 lg:grid lg:grid-cols-12 lg:gap-8">
      ...
      <div class="mt-4">
        ...
        <div class="relative space-y-4 text-sm text-gray-700">
          <!-- title and options -->
          <div class="text-white">
           ...
           <div class="">  
                @if($audio->check_if_premier()->count() != 0)
                      ...
                      @endphp
                    {{-- <p class="text-black">{{ $this->checkPremierDate($premier_date) }}</p> --}}
                      @if($this->checkPremierDate($premier_date) == "Active")
                        ...
                        <div class="w-full bg-center bg-cover h-52" 
                            style="background-image:url({{  $episode_s3_thumbnail }})">
                        </div>

                      @else
                         <!-- podcast-view-media-->
                             @include('livewire.editor.podcast-view.post-media')        
                         <!-- podcast-view-media-->
                      @endif
            ...
            <div class="mt-5 bg-white">
              <div class="">
                ...
                <div class="transition duration-500 ease-in-out sponsorSlides" style="display: none;">
                  <div class="relative flex items-center px-6 py-5 space-x-3 bg-white border border-gray-300 rounded-md shadow-sm hover:border-gray-400 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500 ">
                    <div class="flex-shrink-0">
                      ...
                      <img class="w-12 h-12" src="{{ asset('sponsor/'.$sponsor->audiospon_imgpath) }}" alt="">
                      @else
                      <img class="w-12 h-12" src="{{ asset('images/default_user.jpg') }}" alt="">
                      @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm font-bold text-gray-500 truncate">
	          ...
	          <div class="px-2 pt-2 mt-5 text-black bg-white">
              <div class="flex">
                <div class="flex-auto">
                  <h2 class="m-0 text-xl font-bold">{{ $audio->audio_name }}</h2>     
                  <p>
                    @if($audio->audio_status =='private' )
                        <svg xmlns="http://www.w3.org/2000/svg" class="float-left w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                         <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                        </svg><span class="mt-1 ml-2 text-gray-500 capitalize">Private</span>
                              @else
                          <svg xmlns="http://www.w3.org/2000/svg" class="float-left w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg><span class="ml-2 text-gray-500 capitalize">Public</span>        
                    @endif
                   </p>
                   <p class="mt-5 text-xs text-black uppercase">{{ $audio->created_at }}  | <span>01:37:50 <span>|</span></span> {{ $audio->audio_season }}:{{ $audio->audio_episode }}</p>  
                </div>
                <div>
                  ...
                  <div x-cloak  x-data="{ open: false }" @keydown.escape.stop="open = false" @click.away="open = false" class="relative inline-block text-left">
                    <div>
                      <button type="button" class="flex items-center text-gray-400 bg-gray-100 rounded-full hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500" id="menu-button" aria-expanded="true" aria-haspopup="true" @click="open = !open">
        ...
        <div class="px-2 pt-2 pb-2 bg-white" 
            ...
            <div class="flex justify-between mt-6 space-x-8">
              <div class="flex space-x-6">
                ...
                <span class="inline-flex items-center text-sm">
                  <button class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                    ...
                    <span class="font-medium text-gray-900">{{ $this->getTotalView($audio->id) }}</span>
              ...
              <div class="flex text-sm">
                ...
                @if(Auth::user()->get_plan->check_features('s4')->count() != 0 )
                 <span class="inline-flex items-center mr-3 text-sm" @click="openTab = 6" :class="openTab === 6 ? activeClasses : inactiveClasses">
                  <button class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                     <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" :class="openTab === 6 ? activeClasses : inactiveClasses" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span class="inline-flex items-center mr-3 text-sm" @click="openTab = 2"   >
                  <button class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                   <svg class="w-5 h-5" :class="openTab === 2 ? activeClasses : inactiveClasses" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @else
                  @if(Auth::user()->get_plan->check_features('o4')->count() != 0 )
                    <span class="inline-flex items-center mr-3 text-sm" @click="openTab = 3" >
                      <button class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                     <svg class="w-5 h-5" :class="openTab === 3 ? activeClasses : inactiveClasses"  xmlns="http://www.w3.org/2000/svg"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @else
                  @if(Auth::user()->get_plan->check_features('o5')->count() != 0 )
                    <span class="inline-flex items-center text-sm" @click="openTab = 5" >
                      <button class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                         <svg class="w-5 h-5" :class="openTab === 5 ? activeClasses : inactiveClasses"  xmlns="http://www.w3.org/2000/svg"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                ...
                @else
                  ...
                  <!-- comments -->
                  	<div class="mt-5">
                        ...
                        class="block w-full border-gray-300 rounded-md shadow-sm sm:text-sm" wire:model="comments" 
           ...
           <!-- Notes -->    
            ...
            <!-- notes -->
                <div class="flex mt-5">
                    ...
                    class="flex-1 block w-20 mr-3 border-gray-300 rounded-md shadow-sm sm:text-sm" wire:model="notes_time" >

                    <input type="text" name="email" id="email" placeholder="Notes" 
                    class="block w-full mr-3 border-gray-300 rounded-md shadow-sm sm:text-sm" wire:model="notes_message"  >
                 ...
                 <div class="mt-5">
                    <div class="mt-1">
                      @foreach($audio->get_question as $question)  
                       <div  x-data="{ open: false }">
                          <div class="p-2 mb-3 text-sm bg-gray-100 rounded-md pointer" @click="open = true">
                             {{ $question->qa_question   }}<br><br>
                             <span class="text-xs text-right text-gray-500"><i>Time: {{ $question->qa_time }}</i></span>
                          </div>
                          <div x-show="open" @click.away="open = false">
                              @foreach($question->get_answer as $answerlist)
                                <div class="p-2 mb-3 ml-5 text-xs bg-gray-100 rounded-md">
                             ...
                             <div class="flex my-2">
                                <div class="w-full mr-3">
                                    ...
                                    class="block w-full mr-3 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" wire:model="qa_answer">
                                    @error('qa_answer') 
                     ...
                     <div class="mt-5">
                        <div class="grid grid-cols-9 gap-4">
                          @foreach(Auth::User()->get_playlist as $playlist)  
                            <div class="col-span-3 p-1 bg-white border-2 border-gray-500 rounded-sm">
                              ...
                              <center> 
                                <p class="mt-2 font-bold text-gray-900 text-md">{{ $playlist->playlist_title }}</p> 
         ...
         <!-- transcript & hashtag in tabs -->
          ...
          <div class="mt-5 text-white xl:hidden md:block">
             <div class="space-y-4 top-4">  
                  ...
                  <div class="bg-white shadow">
                    <div class="p-6">
                      ...
                      <div class="flow-root mt-6 text-sm">
                        <p class="text-black truncated">{{ $audio->audio_summary }}</p>
                      </div>
                    </div>
                  </div>
                </section>
            ...
            <div class="p-5 mt-5 mb-5 overflow-y-auto bg-white border-gray-200 lg:block">
                <div class="pb-5">
